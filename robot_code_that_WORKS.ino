...
int pressMode = 4;  // holds values 1-8; start in avoid mode
...
#define OBJECT_TURN_SPEED 160 //turn speed for object following
...
void backRight() { //actually backwards but sensors are on other side of bot
...
void lineFollow() {
  ...
  else if (analogRead(s1) == HIGH && analogRead(s2) == HIGH && analogRead(s3) == HIGH && analogRead(s4) == HIGH && analogRead(s5) == HIGH){
    currentState = STOP;
  }
